...
def main(debug=False):
    ...
    with timer("Saving final datasets"):
        train_df = df[df['TARGET'].notnull()]
        test_df = df[df['TARGET'].isnull()]
        print(f"Final Train shape: {train_df.shape}")
        print(f"Final Test shape: {test_df.shape}")
        train_df.to_csv(os.path.join(PROCESSED_DIR, "train_prepared.csv"), index=False)
        test_df.to_csv(os.path.join(PROCESSED_DIR, "test_prepared.csv"), index=False)
